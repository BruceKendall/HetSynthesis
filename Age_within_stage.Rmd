...
Here, we look at how a non-constant $G$ creates within-stage with a certain structure that reduced the variance in the fraction of individuals maturing, relative to a matrix model.
...
To make the model explicitly stochastic (and to prevent Caswell's ability from righting it off as a "meaningless" deterministic model) we allow the size of each cohort recruiting to the stage to be stochastic.
