...
Here, we look at how a non-constant $G$ creates \textcolor{red}{(BRUCE: something missing here. ``Variance,'' perhaps? Or something deeper or at least more complicated?)} within-stage with a certain structure that reduced the variance in the fraction of individuals maturing, relative to a matrix model.
...
\end{aligned}
$$
How to make sense of this, I'm not sure. But one thing that is clear, the fraction of individuals that is ready to mature has a lot of sources of variability, in contrast to the stochastic matrix models where the only variability is binomial sampling of $x+y$ with fixed probability $g$.

\textcolor{red}{The 4th and 5th terms above - the 2nd line in the expression for $V(Z)$ - are squares of the 2nd and third expressions for $E[Z]$.So maybe explicating this is slightly easier. Maybe.).}
